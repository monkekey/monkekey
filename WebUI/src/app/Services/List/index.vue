<template>
  <div>
    ...
    <List :current="current" :columns="columns" :data="service.services.data.content"
      :total="service.services.data.totalElements"
        ...
        <ListSearch>
          <!-- <Form ref="formInline" inline>
            <Form-item prop="search">
              <Input type="text" placeholder="请输入关键字搜索" v-model="search.serviceName" style="width: 230px;"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleAdd (query) {
          this.$router.push({ path: 'services/form', query: { innCode: this.search.innCode}})
      },
      ...
      handleEdit (id) {
        this.$router.push(`/services/form/${id}`)
      },
      handleDel (id) {
